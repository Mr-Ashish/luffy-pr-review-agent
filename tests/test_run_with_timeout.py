...
class ParseSecondsTests(unittest.TestCase):
    ...
    def test_effective_env(self):
        old = os.environ.get("LUFFY_REVIEW_TIMEOUT_SECONDS")
        try:
            os.environ["LUFFY_REVIEW_TIMEOUT_SECONDS"] = "42"
            self.assertEqual(_mod.effective_seconds(None), 42)
            self.assertEqual(_mod.effective_seconds("10"), 10)
            os.environ["LUFFY_REVIEW_TIMEOUT_SECONDS"] = "off"
            self.assertEqual(_mod.effective_seconds(None), 0)
        finally:
            if old is None:
                os.environ.pop("LUFFY_REVIEW_TIMEOUT_SECONDS", None)
            else:
                os.environ["LUFFY_REVIEW_TIMEOUT_SECONDS"] = old
...
class RunTimeoutTests(unittest.TestCase):
    ...
    def test_cli_resolve_default(self):
        env = {**os.environ}
        env.pop("LUFFY_REVIEW_TIMEOUT_SECONDS", None)
        proc = subprocess.run(
...
class WiringTests(unittest.TestCase):
    def test_hermes_script_mentions_f36(self):
        ...
        self.assertIn("LUFFY_REVIEW_TIMEOUT_SECONDS", text)
