...
class RenderApply(unittest.TestCase):
    def test_render_contains_pack_id(self):
        ...
        self.assertIn("<!-- luffy-lens-pack:security -->", md)
    ...
    def test_apply_to_prompt_rewrites_sections(self):
        ...
        self.assertIn("<!-- luffy-lens-pack:docs -->", out)
...
class CLI(unittest.TestCase):
    def _run(self, *args, env=None):
        e = os.environ.copy()
        e["LUFFY_LENS_PACKS_DIR"] = str(PACKS)
        e.pop("LUFFY_TOGGLES_FILE", None)
        if env:
            e.update(env)
