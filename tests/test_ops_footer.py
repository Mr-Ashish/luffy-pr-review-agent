...
class OpsFooterTests(unittest.TestCase):
    def test_run_url(self):
        u = run_url(
            ...
            repo="Mr-Ashish/luffy-pr-review-agent",
            run_id="12345",
        )
        self.assertEqual(
            u,
            "https://github.com/Mr-Ashish/luffy-pr-review-agent/actions/runs/12345",
        )
    ...
    def test_disabled(self):
        old = os.environ.get("LUFFY_OPS_FOOTER")
        try:
            os.environ["LUFFY_OPS_FOOTER"] = "0"
            self.assertEqual(format_ops_line(repo="a/b", run_id="1"), "")
        finally:
            if old is None:
                os.environ.pop("LUFFY_OPS_FOOTER", None)
            else:
                os.environ["LUFFY_OPS_FOOTER"] = old
    ...
    def test_step_summary(self):
        ...
        self.assertIn("Luffy ops links (F35)", md)
