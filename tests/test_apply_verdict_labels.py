...
all_luffy_labels = _mod.all_luffy_labels
...
class PlanTests(unittest.TestCase):
    def test_approve(self):
        p = plan_labels("APPROVE", True, prefix="luffy")
        self.assertEqual(p["add"], "luffy:approve")
        self.assertIn("luffy:request-changes", p["remove"])
        self.assertIn("luffy:comment", p["remove"])
        self.assertIn("luffy:error", p["remove"])
        self.assertEqual(p["suffix"], "approve")

    def test_request_changes(self):
        p = plan_labels("REQUEST_CHANGES", True, prefix="luffy")
        self.assertEqual(p["add"], "luffy:request-changes")
        self.assertEqual(p["color"], "D93F0B")

    def test_comment(self):
        p = plan_labels("COMMENT", True)
        self.assertEqual(p["add"], "luffy:comment")

    def test_pipeline_fail_forces_error(self):
        p = plan_labels("APPROVE", False, prefix="luffy")
        self.assertEqual(p["add"], "luffy:error")
    ...
    def test_custom_prefix(self):
        ...
        self.assertEqual(all_luffy_labels("bot"), [
            "bot:approve",
            "bot:request-changes",
            "bot:comment",
            "bot:error",
    ...
    def test_label_name(self):
        self.assertEqual(label_name("approve", "luffy"), "luffy:approve")


class EnvTests(unittest.TestCase):
    def test_enabled_default(self):
        old = os.environ.get("LUFFY_PR_LABELS")
        try:
            os.environ.pop("LUFFY_PR_LABELS", None)
            self.assertTrue(enabled_from_env())
            os.environ["LUFFY_PR_LABELS"] = "0"
            self.assertFalse(enabled_from_env())
            os.environ["LUFFY_PR_LABELS"] = "off"
            self.assertFalse(enabled_from_env())
        finally:
            if old is None:
                os.environ.pop("LUFFY_PR_LABELS", None)
            else:
                os.environ["LUFFY_PR_LABELS"] = old
...
class CliTests(unittest.TestCase):
    ...
    def test_plan_json(self):
        ...
        self.assertEqual(data["add"], "luffy:request-changes")
    ...
    def test_apply_fixture(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            cp = self._run(
                ...
                "--pipeline-ok",
                "true",
                env={"LUFFY_LABELS_FIXTURE": str(fix)},
            )
            self.assertEqual(cp.returncode, 0, cp.stderr)
            data = json.loads(cp.stdout)
            ...
            self.assertEqual(plan["add"], "luffy:comment")
...
class WiringTests(unittest.TestCase):
    def test_report_verdict_wires_f37(self):
        ...
        self.assertIn("LUFFY_PR_LABELS", text)
    ...
    def test_workflow_exports_var(self):
        text = (ROOT / ".github" / "workflows" / "luffy-review-reusable.yml").read_text()
        self.assertIn("LUFFY_PR_LABELS", text)
