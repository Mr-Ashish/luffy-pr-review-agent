#!/usr/bin/env python3
"""F26: default LUFFY_MODEL is a single source of truth across script + ops docs."""
...
def _script_default() -> str:
    ...
    m = re.search(r'^DEFAULT_LUFFY_MODEL="([^"]+)"', text, re.M)
    if not m:
        raise AssertionError("DEFAULT_LUFFY_MODEL not found in run-hermes-review.sh")
    return m.group(1)
...
class DefaultModelAlignmentTests(unittest.TestCase):
    ...
    def test_env_example_does_not_force_mini(self):
        ...
        self.assertNotRegex(
            env,
            r"(?m)^LUFFY_MODEL=openai/gpt-5-mini\s*$",
    ...
    def test_workflow_defers_empty_model_to_script(self):
        wf = (ROOT / ".github" / "workflows" / "luffy-review-reusable.yml").read_text()
        # Old pattern re-hardcoded a second default
        self.assertNotIn(
            'export LUFFY_MODEL="${LUFFY_MODEL:-anthropic/claude-opus-5}"',
            wf,
        )
