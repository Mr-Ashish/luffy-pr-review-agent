...
def _run(
    ...
    base.pop("LUFFY_COOLDOWN_FORCE", None)
    if "LUFFY_COOLDOWN_FORCE" in (env or {}):
        base["LUFFY_COOLDOWN_FORCE"] = env["LUFFY_COOLDOWN_FORCE"]
    base["NOW_EPOCH"] = extra.get("NOW_EPOCH", str(NOW))
    if "LUFFY_COOLDOWN_SECONDS" not in base and "LUFFY_COOLDOWN_SECONDS" not in (env or {}):
        base["LUFFY_COOLDOWN_SECONDS"] = "900"

    with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as f:
        json.dump(comments if comments is not None else [], f)
        fixture = f.name
    try:
        base["LUFFY_COOLDOWN_FIXTURE"] = fixture
...
def _fail_body(pr: str = "42") -> str:
    return (
        ...
        f"## Luffy Review — PR #{pr}\n\n"
        "**Verdict:** COMMENT\n"
        "### Summary\nLuffy failed to produce a review (hermes exit 1).\n"
        "### What I checked\n- Failure path only\n"
    )
...
class CooldownCheckTests(unittest.TestCase):
    ...
    def test_recent_success_blocks(self):
        r = _run(
            ...
            env={"LUFFY_COOLDOWN_SECONDS": "900"},
        )
