...
def _run(
    ...
    *,
    ...
    env.pop("LUFFY_PR_REVIEWS_FIXTURE", None)
    env.pop("LUFFY_REPLACE_PREVIOUS", None)
    if fixture is not None:
        env["LUFFY_PR_REVIEWS_FIXTURE"] = str(fixture)
    if env_extra:
...
class DismissPriorPrReviewsTests(unittest.TestCase):
    def test_replace_off_skips(self):
        r = _run(env_extra={"LUFFY_REPLACE_PREVIOUS": "0"})
        ...
        reviews = [
            {
                "id": 11,
                "state": "APPROVED",
                "body": "old\n<!-- luffy-pr-review pr=7 run=1 -->",
            },
            {
                "id": 22,
                "state": "CHANGES_REQUESTED",
                "body": "## Luffy\n<!-- luffy-pr-review pr=7 run=2 -->",
            },
            {
                "id": 33,
                "state": "COMMENTED",
                "body": "short\n<!-- luffy-pr-review pr=7 run=3 -->",
            },
            {
                "id": 44,
                "state": "APPROVED",
        ...
        reviews = [
            {
                "id": 99,
                "state": "CHANGES_REQUESTED",
                "body": "<!-- luffy-pr-review pr=99 run=x -->",
            }
        ]
        with tempfile.TemporaryDirectory() as td:
            p = Path(td) / "reviews.json"
    ...
    def test_paginated_concat_arrays(self):
        ...
        page1 = [{"id": 1, "state": "APPROVED", "body": "<!-- luffy-pr-review pr=7 -->"}]
        page2 = [
            {
                "id": 2,
                "state": "CHANGES_REQUESTED",
                "body": "<!-- luffy-pr-review pr=7 -->",
            }
        ]
