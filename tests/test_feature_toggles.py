...
class ResolvePrecedence(unittest.TestCase):
    def test_default(self):
        ...
        self.assertEqual(r.env, "LUFFY_FIXIT_PROMPTS")

    def test_env_wins_over_file(self):
        r = _mod.resolve(
            "fixit_prompts",
            env={"LUFFY_FIXIT_PROMPTS": "0"},
    ...
    def test_file_when_env_unset(self):
        r = _mod.resolve(
            "fixit_prompts",
            env={},
            file_map={"LUFFY_FIXIT_PROMPTS": 0},
            load_file=False,
        )
        self.assertFalse(r.value)
...
class FileLoad(unittest.TestCase):
    ...
    def test_search_luffy_dir(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / ".luffy").mkdir()
            (root / ".luffy" / "toggles.json").write_text(
...
class CLI(unittest.TestCase):
    ...
    def test_shell_exports(self):
        ...
        self.assertEqual(cp.returncode, 0, cp.stderr)
        self.assertIn("export LUFFY_FIXIT_PROMPTS=", cp.stdout)
