...
class CliTests(unittest.TestCase):
    def _run(self, *args: str, env: dict | None = None) -> subprocess.CompletedProcess:
        ...
        e.pop("LUFFY_SKIP_PATHS_FORCE", None)
        if env:
            e.update(env)
    ...
    def test_cli_docs_skip(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            cp = self._run(
                "--paths-file",
                str(pf),
                env={"LUFFY_SKIP_PATH_GLOBS": "docs"},
            )
    ...
    def test_cli_disabled_default(self):
        env = {**os.environ}
        env.pop("LUFFY_SKIP_PATH_GLOBS", None)
        env.pop("LUFFY_SKIP_PATHS_FORCE", None)
        cp = subprocess.run(
    ...
    def test_force(self):
        cp = self._run(
            "--path",
            "README.md",
            env={"LUFFY_SKIP_PATH_GLOBS": "docs", "LUFFY_SKIP_PATHS_FORCE": "1"},
        )
        self.assertEqual(cp.returncode, 0)
...
class WiringTests(unittest.TestCase):
    def test_workflow_mentions_f38(self):
        ...
        self.assertIn("LUFFY_SKIP_PATH_GLOBS", text)
