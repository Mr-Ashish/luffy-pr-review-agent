#!/usr/bin/env python3
"""F28: repo-local .luffy/ ingest, hub skip, preload preference."""
...
class LocalIngestTests(unittest.TestCase):
    def test_local_layout_writes_dot_luffy(self):
        with tempfile.TemporaryDirectory() as td:
            target = Path(td)
            payload = {"run": _sample_run()}
            env = os.environ.copy()
            env["CLIENT_PAYLOAD"] = json.dumps(payload)
            env["LUFFY_INGEST_LAYOUT"] = "local"
            env["LUFFY_MEMORY_ROOT"] = str(target)
            env["LUFFY_MEMORY_PATH"] = ".luffy"
            subprocess.check_call([sys.executable, str(INGEST)], env=env, cwd=str(target))
            mem = target / ".luffy" / "MEMORY.md"
            self.assertTrue(mem.exists(), "MEMORY.md under .luffy/")
            self.assertIn("REQUEST CHANGES", mem.read_text())
            run_meta = target / ".luffy" / "runs" / "pr7-run123-a1" / "meta.json"
            ...
            review = target / ".luffy" / "runs" / "pr7-run123-a1" / "review.md"
            summary = target / ".luffy" / "runs" / "pr7-run123-a1" / "summary.md"
    ...
    def test_local_no_slug_required(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue((target / ".luffy" / "MEMORY.md").exists())
...
class HubPublishGateTests(unittest.TestCase):
    def test_hub_skipped_when_mode_local(self):
        env = os.environ.copy()
        env["LUFFY_MEMORY_MODE"] = "local"
        # Ensure explicit publish not set
        env.pop("LUFFY_HUB_PUBLISH", None)
    ...
    def test_hub_runs_when_publish_explicit(self):
        """With LUFFY_HUB_PUBLISH=1 but missing payload tooling path still past gate."""
        env = os.environ.copy()
        env["LUFFY_MEMORY_MODE"] = "local"
        env["LUFFY_HUB_PUBLISH"] = "1"
        env["GITHUB_TOKEN"] = "fake"
        ...
        combined = r.stdout + r.stderr
        self.assertNotIn("LUFFY_HUB_PUBLISH=0", combined)
            ...
            def fake_curl(cmd, *a, **kw):
                ...
                for i, c in enumerate(cmd):
                    if c == "-o" and i + 1 < len(cmd):
                        out_file = cmd[i + 1]
                calls.append(url)
                if ".luffy/MEMORY.md" in url or "%2Fluffy" in url:
...
if [[ -n "${{out:-}}" ]]; then
  if [[ "$*" == *".luffy/MEMORY.md"* ]] || [[ "$*" == *"%2E%6Cuffy"* ]]; then
...
body = ""
code = "404"
if url and ".luffy/MEMORY.md" in url:
    body = "# Local memory\\n\\nfrom .luffy\\n"
    code = "200"
...
"""
            ...
            env["LUFFY_MEMORY_MODE"] = "both"  # hub available but local must win
            env["LUFFY_HUB_PUBLISH"] = "1"
            env["GITHUB_TOKEN"] = "fake"
            r = subprocess.run(
                ["bash", str(PRELOAD)],
            ...
            self.assertIn("from .luffy", dest.read_text())
