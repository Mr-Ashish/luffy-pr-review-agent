...
class FromThreads(unittest.TestCase):
    def test_reply_under_luffy_root(self):
        comments = [
            {
                "id": 10,
                "path": "pkg/foo.go",
...
class AssembleFixture(unittest.TestCase):
    def test_assemble_fixture(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            out.mkdir()
            env = os.environ.copy()
            env["LUFFY_FP_RESOLVE"] = "1"
            env["LUFFY_FP_RESOLVE_FIXTURE"] = str(fixture)
            env["LUFFY_FP_RESOLVE_ISSUE_FIXTURE"] = str(td_path / "empty.json")
            (td_path / "empty.json").write_text("[]", encoding="utf-8")
            # isolate toggle registry
            old = os.environ.get("LUFFY_FP_RESOLVE")
            os.environ["LUFFY_FP_RESOLVE"] = "1"
            os.environ["LUFFY_FP_RESOLVE_FIXTURE"] = str(fixture)
            os.environ["LUFFY_FP_RESOLVE_ISSUE_FIXTURE"] = str(td_path / "empty.json")
            try:
                r = _mod.assemble(
                ...
                if old is None:
                    ...
                    os.environ["LUFFY_FP_RESOLVE"] = old
                os.environ.pop("LUFFY_FP_RESOLVE_FIXTURE", None)
                os.environ.pop("LUFFY_FP_RESOLVE_ISSUE_FIXTURE", None)
