...
3. **Normalize** `--head-sha` stamps `<!-- luffy-review pr=N run=R head=SHA -->`.
4. **Toggle** `LUFFY_INCREMENTAL` default **off** (safe).
...
LUFFY_INCREMENTAL=1 python3 scripts/incremental_review.py plan --pr 1 --head-sha bbb --last-head aaa
```
