...
author prose; Luffy needs a **safe, code-first** scaffold.
...
2. **Markers** `<!-- luffy-description -->` … `<!-- /luffy-description -->`.
3. **Modes:** `fill-empty` (default) | `markers` | `force`.
4. **Assemble** soft-writes `pr-description.md` + merged body; never auto-posts.
5. **Apply** CLI: `gh pr edit` only if `LUFFY_PR_DESCRIPTION_APPLY=1` or `--force-apply`.
