# F68 + F69 — Agent tools pipeline + Luffy-native self-evolution
...
Tool/toolset choices were implicit (`LUFFY_TOOLSETS=terminal`) with no durable
...
| **toolsets** | Print comma-separated toolsets for `LUFFY_TOOLSETS` |

Orchestrator loads active toolsets when `LUFFY_TOOLSETS` unset. Auto-adopt remains
opt-in (`LUFFY_AGENT_TOOLS_AUTO_ADOPT=0` default). Research stage opt-in via
`LUFFY_AGENT_TOOLS_RESEARCH=1`.

## F69 — Hermes best practices as Luffy-native self-evolution

### Problem

Hermes self-evolution (skill files, GEPA/DSPy, trajectory datasets) is valuable
but must **not** be a Hermes fork. Luffy needed a control-plane native loop.

### Patterns adopted (from ROI H3/H9/H10)

| Hermes idea | Luffy-native |
|-------------|--------------|
...
| **inject** | Assemble-time prompt injection (`<!-- luffy-f69-skills -->`) |

Orchestrator always soft-ingests after save_trace. Auto propose/eval when
`LUFFY_SELF_EVOLVE=1`. Adopt stays human-gated.
