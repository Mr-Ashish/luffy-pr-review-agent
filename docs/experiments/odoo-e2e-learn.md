...
Artifacts: `.luffy-out-e2e-pr2-f44/` (review, raw, run-bundle, traces/pr2-runlocal-a1).
...
- Prefer fixing hermes -z path so chat fallback is rare.
- Soft-nudge / force at least one terminal read of changed files when tool_turns=0 and diff multi-file.
- Investigate SOUL.md injection false positive with hermes pin.
