...
cannot filter project boards / search / automation on Luffy outcome without
parsing comment text.

## Ship

- `scripts/apply-verdict-labels.py` plan|apply
- Labels: `{prefix}:approve|request-changes|comment|error` (default prefix `luffy`)
- Pipeline fail → always `error` (never green-wash APPROVE)
- Soft-fail; opt-out `LUFFY_PR_LABELS=0`
