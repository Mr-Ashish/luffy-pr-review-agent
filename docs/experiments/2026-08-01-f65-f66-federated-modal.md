...
1. **`LUFFY_MEMORY_TENANT`** (optional string). Empty = classic
...
shipped in F54–F64 (`LUFFY_LENS_PACK=auto`, tenant, hub knobs). Docs still
framed GHA as primary kitchen.

### Fix

1. Modal `review_pr` env: **`LUFFY_LENS_PACK=auto`**, lens packs on, F65
...
```bash
# F65 tenant ingest
LUFFY_MEMORY_TENANT=demo HUB_ROOT=/tmp/hub CLIENT_PAYLOAD='{"run":{...}}' \
