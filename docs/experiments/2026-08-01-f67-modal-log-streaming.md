...
6. Full traces were copied to Volume `luffy-traces` but **not** visible live in the UI.
...
3. **`LUFFY_STREAM_LOGS=1`** (set on Modal): `run-hermes-review.sh` tees Hermes
   stderr to file **and** process stderr.
4. **Post-run `_emit_artifact_summary`**: agent-loop tool stats + log tails.
5. Volume still stores full `.luffy-out` + `modal-run-index.json`.
