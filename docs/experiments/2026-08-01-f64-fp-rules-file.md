...
through local `.luffy/` publish → next-run assemble.

## Fix

1. **`fp-rules.json`** schema (`schema_version=1`, `rules[]`) written next to
   MEMORY on F62 `update` and under `OUT_DIR`.
2. **Assemble** loads durable rules (OUT_DIR / HERMES / `LUFFY_FP_RULES_FILE`)
   into the FP plan alongside thread + MEMORY signals.
3. **Preload** fetches `.luffy/fp-rules.json` into Hermes + OUT_DIR (soft).
