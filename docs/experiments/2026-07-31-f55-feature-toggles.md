...
Product gates (`LUFFY_FIXIT_PROMPTS`, `LUFFY_ISSUE_CONTEXT`, inline, labels, …)
...
1. **`scripts/feature_toggles.py`** — registry of short keys + `LUFFY_*` env +
   kind/default/category/feature id. Precedence: **env > file > default**.
2. **File overrides:** `LUFFY_TOGGLES_FILE` or `.luffy/toggles.json` (JSON map of
