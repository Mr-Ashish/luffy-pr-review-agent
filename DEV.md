...
- F24 dismiss only covers GitHub-dismissible states (`APPROVED`, `CHANGES_REQUESTED`). Prior `COMMENTED` Luffy reviews (including APPROVE soft-fallback and pipeline-fail COMMENT) **cannot** be dismissed via the API and may stack in the Reviews panel — expected limitation, not a bug in `dismiss-prior-pr-reviews.sh`.
- F24 reuses the paginated JSON walk from F19 (`json.JSONDecoder().raw_decode`) because `gh api --paginate` concatenates arrays; a plain `json.loads` would miss all but the first page of reviews.
