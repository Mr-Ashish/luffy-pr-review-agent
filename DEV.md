...
- **F22 verdict signal** is a post-post decoration: `scripts/parse-verdict.py` reads `**Verdict:**` from the posted review and maps APPROVE→`+1`/`success`, REQUEST CHANGES→`-1`/`failure`, COMMENT→`eyes`/`success`; pipeline_rc≠0 forces `-1`/`error`. `scripts/report-verdict.sh` applies soft reaction + commit status `luffy/review` on the PR head SHA and writes a job-summary section. Opt-out `LUFFY_COMMIT_STATUS=0`. Required checks can require context `luffy/review`.
