...
DESCRIPTIONS = {
    "approve": "Luffy verdict: APPROVE",
    "request-changes": "Luffy verdict: REQUEST CHANGES",
    "comment": "Luffy verdict: COMMENT",
    "error": "Luffy pipeline error or unknown verdict",
}


def enabled_from_env() -> bool:
    v = (os.environ.get("LUFFY_PR_LABELS") or "1").strip().lower()
    return v not in ("0", "false", "off", "no")


def label_prefix() -> str:
    p = (os.environ.get("LUFFY_LABEL_PREFIX") or "luffy").strip()
    # sanitize: allow alnum, hyphen, underscore
    p = re.sub(r"[^A-Za-z0-9_-]", "", p) or "luffy"
    return p
...
def all_luffy_labels(prefix: str | None = None) -> list[str]:
    pref = prefix if prefix is not None else label_prefix()
...
def plan_labels(
    ...
    *,
    ...
    return {
        ...
        "description": DESCRIPTIONS.get(suffix, "Luffy verdict label"),
    }
...
def apply_labels(
    ...
    *,
    ...
    fixture = (os.environ.get("LUFFY_LABELS_FIXTURE") or "").strip()
    result: dict[str, Any] = {
        "ok": True,
        "posted": False,
        "add": plan["add"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    def add_common(sp: argparse.ArgumentParser) -> None:
        ...
        sp.add_argument("--prefix", default=None, help="override LUFFY_LABEL_PREFIX")
    ...
    if args.prefix:
        os.environ["LUFFY_LABEL_PREFIX"] = args.prefix
