#!/usr/bin/env python3
"""F59: incremental PR review — scope diff to commits since last Luffy review.

Deterministic control-plane. Finds last reviewed head SHA from prior Luffy
comment markers (`head=`), then replaces the assembled full PR diff with a
compare-patch base...head when LUFFY_INCREMENTAL=1.

Usage:
  python3 scripts/incremental_review.py parse-marker '<!-- luffy-review pr=3 head=abc -->'
  python3 scripts/incremental_review.py plan --pr-json pr.json --comments-json c.json
  python3 scripts/incremental_review.py assemble --repo o/r --pr 3 --out-dir .luffy-out

Env:
  LUFFY_INCREMENTAL=0 (default off) | 1 — enable incremental scoping
  LUFFY_INCREMENTAL_FIXTURE=path.json — hermetic plan/assemble (no network)
  LUFFY_INCREMENTAL_MIN_COMMITS=1 — min new commits to stay incremental
"""
...
# <!-- luffy-review pr=12 run=99 head=deadbeef -->
_MARKER_RE = re.compile(
    r"<!--\s*luffy-review\s+pr=(?P<pr>\d+)"
    r"(?:\s+run=(?P<run>[^\s>]+))?"
...
def enabled(raw: str | None = None) -> bool:
    try:
        ...
        v = raw if raw is not None else os.environ.get("LUFFY_INCREMENTAL")
...
def parse_marker(body: str) -> dict[str, str | None] | None:
    ...
    if not m:
        # prefix-only legacy
        if "<!-- luffy-review pr=" in body:
            m2 = re.search(r"<!--\s*luffy-review\s+pr=(\d+)", body, re.I)
            if m2:
...
def format_marker(pr: str | int, *, run: str | None = None, head: str | None = None) -> str:
    ...
    return f"<!-- luffy-review {' '.join(parts)} -->"
...
def extract_last_head_from_comments(
    ...
    """Newest success Luffy review with a head= SHA."""
    pr_s = str(pr)
...
def render_note(plan_d: dict[str, Any]) -> str:
    ...
    if mode == "unchanged":
        return (
            ...
            "matches last Luffy review. Prefer a short COMMENT confirming no new diff, "
...
def assemble(
    *,
    ...
    fixture_path = (os.environ.get("LUFFY_INCREMENTAL_FIXTURE") or "").strip()
