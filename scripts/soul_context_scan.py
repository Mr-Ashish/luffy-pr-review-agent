...
(replaces content with a [BLOCKED: …] placeholder). Luffy's F44 e2e log:
...
Env:
  LUFFY_SOUL_SCAN=1 (default) | 0/off to skip in shell wrappers
"""
...
def enabled() -> bool:
    v = (os.environ.get("LUFFY_SOUL_SCAN") or "1").strip().lower()
...
def main(argv: list[str] | None = None) -> int:
    ...
    c.add_argument(
        "--force",
        action="store_true",
        help="Scan even when LUFFY_SOUL_SCAN=off",
    )
