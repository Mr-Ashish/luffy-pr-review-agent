...
agent/packs/*.json (or LUFFY_LENS_PACKS_DIR).
...
def packs_enabled(raw: str | None = None) -> bool:
    """LUFFY_LENS_PACKS=1 (default) | 0/off. Prefer F55 registry when present."""
    try:
        from feature_toggles import is_enabled  # type: ignore

        return bool(is_enabled("lens_packs"))
    except Exception:
        v = raw if raw is not None else os.environ.get("LUFFY_LENS_PACKS")
        return _truthy(v, default=True)


def default_packs_dir() -> Path:
    env = (os.environ.get("LUFFY_LENS_PACKS_DIR") or "").strip()
    if env:
        return Path(env)
    root = Path(os.environ.get("LUFFY_ROOT") or Path(__file__).resolve().parents[1])
...
def active_pack_id(raw: str | None = None) -> str:
    try:
        ...
        pass
    v = (raw if raw is not None else os.environ.get("LUFFY_LENS_PACK") or "default")
    v = str(v).strip().lower() or "default"
    return v
