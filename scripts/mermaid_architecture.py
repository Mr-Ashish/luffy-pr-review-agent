...
Env:
  LUFFY_MERMAID=1 (default) | 0/off
  LUFFY_MERMAID_MAX_NODES=24
  LUFFY_MERMAID_MAX_FILES_PER_GROUP=6
"""
...
def enabled(raw: str | None = None) -> bool:
    try:
        ...
        v = raw if raw is not None else os.environ.get("LUFFY_MERMAID")
...
def max_nodes() -> int:
    try:
        ...
        pass
    return _int_env("LUFFY_MERMAID_MAX_NODES", 24)


def max_files_per_group() -> int:
    return _int_env("LUFFY_MERMAID_MAX_FILES_PER_GROUP", 6)
...
def apply_to_review(review: str, section: str) -> str:
    ...
    if "<!-- luffy-mermaid -->" in review or "### Architecture diagram" in review:
...
def main(argv: list[str] | None = None) -> int:
    ...
    pa.add_argument("--force", action="store_true", help="ignore LUFFY_MERMAID=0")
