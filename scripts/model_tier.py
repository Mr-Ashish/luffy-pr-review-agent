...
first. Operators opt in with LUFFY_MODEL_TIER=auto (default off keeps F26
...
Env:
  LUFFY_MODEL_TIER          off|auto|cheap|full  (default off)
  LUFFY_MODEL               when tier=off: single model; when auto: full model
  LUFFY_MODEL_FULL          full-tier model (default anthropic/claude-opus-5)
  LUFFY_MODEL_CHEAP         cheap-tier model (default openai/gpt-4.1-mini)
  LUFFY_TIER_MAX_BYTES      tiny-diff threshold (default 12000)
  LUFFY_TIER_MAX_FILES      tiny file-count threshold (default 3)
  OPENROUTER_MODEL          fallback when LUFFY_MODEL unset (tier=off)
...
def resolve_models(
    *,
    ...
    if not full:
        full = (luffy_model or "").strip() or (openrouter_model or "").strip() or DEFAULT_FULL_MODEL
    return full, cheap
...
def select_model(
    *,
    ...
    if mode == "off":
        ...
        explicit = (luffy_model or "").strip() or (openrouter_model or "").strip()
        if explicit:
            return {
                **base,
                "model": explicit,
...
def cmd_select(args: argparse.Namespace) -> int:
    ...
    mode = args.mode if args.mode is not None else os.environ.get("LUFFY_MODEL_TIER")

    result = select_model(
        mode=parse_mode(mode),
        luffy_model=os.environ.get("LUFFY_MODEL"),
        openrouter_model=os.environ.get("OPENROUTER_MODEL"),
        full_model=os.environ.get("LUFFY_MODEL_FULL"),
        cheap_model=os.environ.get("LUFFY_MODEL_CHEAP"),
        diff_bytes=int(diff_bytes or 0),
        file_count=int(file_count or 0),
        paths=paths,
        diff_truncated=diff_truncated,
        max_bytes=_positive_int(os.environ.get("LUFFY_TIER_MAX_BYTES"), DEFAULT_MAX_BYTES),
        max_files=_positive_int(os.environ.get("LUFFY_TIER_MAX_FILES"), DEFAULT_MAX_FILES),
    )
