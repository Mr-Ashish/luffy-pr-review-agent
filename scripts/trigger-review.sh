#!/usr/bin/env bash
# F32: unified entry to start a Luffy review (local | modal | print commands).
#
# Usage:
...
#
# Env:
#   LUFFY_MODEL / OPENROUTER_MODEL  default model (local uses script default if unset)
...
usage() {
  cat <<'EOF'
trigger-review.sh — start a Luffy PR review
...
MODEL="${LUFFY_MODEL:-${OPENROUTER_MODEL:-}}"
POST="${POST_COMMENT:-}"
NO_POST=0
CHEAP=0
...
case "$MODE" in
  print)
    ...
    local_cmd
    echo "# modal cheap worker (needs modal auth + secrets luffy-openrouter, luffy-github)"
    modal_cmd
