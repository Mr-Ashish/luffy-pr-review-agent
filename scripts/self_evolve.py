#!/usr/bin/env python3
"""F69: Luffy-native self-evolution (Hermes best practices, not a Hermes fork).
...
Env:
  LUFFY_ROOT
  LUFFY_SELF_EVOLVE=0|1          (default 0 for auto-propose in CI; CLI always works)
  LUFFY_SELF_EVOLVE_AUTO_ADOPT=0|1  (default 0)
  LUFFY_EVOLUTION_ROOT           (default: <root>/memory/evolution)
"""
...
def _root() -> Path:
    env = (os.environ.get("LUFFY_ROOT") or "").strip()
    if env:
...
def _evo_root(root: Path) -> Path:
    env = (os.environ.get("LUFFY_EVOLUTION_ROOT") or "").strip()
    if env:
        return Path(env).resolve()
...
def cmd_inject(args: argparse.Namespace) -> int:
    ...
    if (os.environ.get("LUFFY_SELF_EVOLVE") or "1").strip().lower() in (
        "0",
        "false",
        "off",
        "no",
    ...
    blocks: list[str] = []
    if skills:
        parts = ["## Evolved skills (F69 — Luffy-native; treat as reviewer discipline)\n"]
        for sp in skills[:8]:
    ...
    if "<!-- luffy-f69-skills -->" in original:
        # replace previous injection
        new = re.sub(
            r"<!-- luffy-f69-skills -->.*?<!-- /luffy-f69-skills -->\n?",
            f"<!-- luffy-f69-skills -->\n{injection}<!-- /luffy-f69-skills -->\n",
            original,
            count=1,
            flags=re.DOTALL,
        )
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="F69 Luffy-native self-evolution")
