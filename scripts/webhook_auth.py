...
def extract_bearer(headers: Mapping[str, str]) -> str:
    ...
    if auth.lower().startswith("bearer "):
        return auth[7:].strip()
    return (h.get("x-luffy-token") or "").strip()
...
def authorize_webhook(
    ...
    *,
    ...
    if sig:
        if not secret:
            return {
                "ok": False,
                "auth": "denied",
                "error": "signature present but LUFFY_WEBHOOK_SECRET unset",
            }
    ...
    # Simple API path — bearer / X-Luffy-Token
    if token:
        ...
        return {
            "ok": False,
            "auth": "denied",
            "error": "missing or invalid bearer token (Authorization: Bearer … or X-Luffy-Token)",
        }
    ...
    return {
        "ok": False,
        "auth": "denied",
        "error": "LUFFY_WEBHOOK_SECRET set; require X-Hub-Signature-256 (or also set LUFFY_WEBHOOK_TOKEN for API)",
    }
