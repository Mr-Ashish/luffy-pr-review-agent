...
case "$cmd" in
  ...
  summary)
    ...
    mode="$(get MEMORY_MODE "${LUFFY_MEMORY_MODE:-local}")"
    path="$(get MEMORY_PATH "${LUFFY_MEMORY_PATH:-.luffy}")"
    echo "### Luffy memory health (F30)"
    echo ""
    echo "| Field | Value |"
    echo "|-------|-------|"
    echo "| **Mode** | \`${mode}\` |"
    ...
    case "$local_pub" in
      failed|no_token|error|missing_repo)
        echo "> ⚠️ **Local \`.luffy/\` memory did not land.** Next review may not see this run's notes."
        echo "> Check branch protection (bot push to default branch), \`contents: write\`, and job logs for \`LOCAL_PUBLISH=\`."
        echo ""
        ;;
      ok|noop|skipped)
        echo "> Local memory stage: **${local_pub}** (slim pack under \`.luffy/\`; fat traces remain artifacts)."
        echo ""
        ;;
      *)
  ...
  warn-if-bad)
    ...
    case "$local_pub" in
      failed|no_token|error|missing_repo)
        echo "::warning::F30 local .luffy memory publish=${local_pub} — durable memory may be stale (branch protection / token / push). See job summary Memory health."
        ;;
    esac
    ;;
  *)
