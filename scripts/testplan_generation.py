...
Env:
  LUFFY_TESTPLAN=1 (default) | 0/off
  LUFFY_TESTPLAN_MAX_CASES=12
"""
...
def enabled(raw: str | None = None) -> bool:
    try:
        ...
        v = raw if raw is not None else os.environ.get("LUFFY_TESTPLAN")
...
def max_cases() -> int:
    try:
        ...
        pass
    raw = (os.environ.get("LUFFY_TESTPLAN_MAX_CASES") or "").strip()
    if raw:
        try:
...
def apply_to_review(review: str, section: str, *, force: bool = False) -> str:
    ...
    """
    has_marker = "<!-- luffy-testplan -->" in review
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name, help_ in (
        ...
        sp.add_argument("--force", action="store_true", help="ignore LUFFY_TESTPLAN=0")
    ...
    if args.cmd != "apply" and not args.force and not enabled():
        sys.stderr.write("testplan disabled (LUFFY_TESTPLAN=0)\n")
        return 0
    if args.cmd == "apply" and not args.force and not enabled():
        return 0
