...
Precedence (highest first):
  1. process environment (`LUFFY_*`)
  2. file overrides (LUFFY_TOGGLES_FILE or `.luffy/toggles.json`)
  3. registry default
...
REGISTRY: list[ToggleSpec] = [
    ...
    ToggleSpec(
        ...
        True,
        "product",
        "Apply verdict labels (luffy:approve / request-changes / …)",
        "F37",
    ),
    ToggleSpec(
        "pr_review",
...
def parse_bool(raw: str | None, default: bool) -> bool:
    ...
    if s in _TRUTHY:
        return True
    # unknown non-empty → treat as true (matches existing Luffy scripts)
    return True
...
def load_file_overrides(
    ...
    *,
    ...
    """Load JSON map of short keys and/or LUFFY_* env names → values.

    Search order when path is None:
      LUFFY_TOGGLES_FILE env, then <root>/.luffy/toggles.json for each root.
    """
    candidates: list[Path] = []
    if path:
        candidates.append(Path(path))
    else:
        env_path = (os.environ.get("LUFFY_TOGGLES_FILE") or "").strip()
        if env_path:
            candidates.append(Path(env_path))
        roots = list(search_roots or [])
        if not roots:
            # LUFFY_ROOT, cwd, script-relative repo root
            lr = (os.environ.get("LUFFY_ROOT") or "").strip()
            if lr:
        ...
        for r in roots:
            ...
            if rp in seen:
                continue
            seen.add(rp)
            candidates.append(rp / ".luffy" / "toggles.json")
...
def resolve(
    ...
    *,
    ...
    """Resolve one toggle by short key or LUFFY_* env name."""
    spec = _BY_KEY.get(key_or_env) or _BY_ENV.get(key_or_env)
    if spec is None:
        # allow bare LUFFY_* unknown — return str from env with source
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="F55 feature toggle registry (env + .luffy/toggles.json)"
    )
    ...
    pg.add_argument("name", help="short key or LUFFY_* env name")
    ...
    ps = sub.add_parser("shell", help="print export LUFFY_*=… lines")
