#!/usr/bin/env bash
# F24: Dismiss prior Luffy formal PR reviews on the same PR (Reviews-panel hygiene).
#
# Finds reviews whose body contains `<!-- luffy-pr-review pr=N` and dismisses
...
#   LUFFY_REPLACE_PREVIOUS — 1 (default) to dismiss; 0/off to skip
#   LUFFY_PR_REVIEWS_FIXTURE — optional JSON array of {id,state,body} (no network)
#
...
case "${REPLACE}" in
  0|false|FALSE|off|OFF|no|NO)
    log "F24 skip dismiss (LUFFY_REPLACE_PREVIOUS=$REPLACE)"
...
marker="<!-- luffy-pr-review pr=${PR_NUMBER}"
...
if [[ -n "${LUFFY_PR_REVIEWS_FIXTURE:-}" && -f "${LUFFY_PR_REVIEWS_FIXTURE}" ]]; then
  log "F24 using fixture $LUFFY_PR_REVIEWS_FIXTURE"
  SEL="$(python3 -c "$select_py" <"$LUFFY_PR_REVIEWS_FIXTURE" 2>/dev/null || true)"
else
...
if [[ "$skipped" -gt 0 ]]; then
  log "F24 $skipped prior Luffy COMMENTED review(s) cannot be dismissed (API limit)"
fi

dismissed=0
if [[ -n "${LUFFY_PR_REVIEWS_FIXTURE:-}" ]]; then
  ...
  for rid in $DISMISSABLE; do
    ...
    if gh api --method PUT \
      ...
      -f message="Superseded by newer Luffy review" \
      -f event="DISMISS" >/dev/null 2>&1; then
      log "F24 dismissed prior Luffy review id=$rid"
      dismissed=$((dismissed + 1))
    else
