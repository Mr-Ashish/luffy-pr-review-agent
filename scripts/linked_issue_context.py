...
Env:
  LUFFY_ISSUE_CONTEXT          1 (default) | 0/off
  LUFFY_ISSUE_CONTEXT_MAX      max issues to fetch (default 3)
  LUFFY_ISSUE_CONTEXT_COMMENTS max comments per issue (default 8)
  LUFFY_ISSUE_BODY_CHARS       max body chars per issue (default 4000)
  LUFFY_ISSUE_COMMENT_CHARS    max chars per comment (default 800)
  LUFFY_ISSUE_CONTEXT_FIXTURE  path to JSON list of issue objects (no network)
  LUFFY_ISSUE_FROM_BRANCH      1 (default) | 0 — extract N from head branch
...
def extract_issue_refs(
    *,
    ...
    max_n = (
        ...
        else _int_env("LUFFY_ISSUE_CONTEXT_MAX", _DEFAULT_MAX)
    )
    if max_n <= 0:
        return []
    if from_branch is None:
        from_branch = _truthy(os.environ.get("LUFFY_ISSUE_FROM_BRANCH"), default=True)
...
def format_issue_section(
    ...
    *,
    ...
    body_lim = (
        ...
        else _int_env("LUFFY_ISSUE_BODY_CHARS", _DEFAULT_BODY_CHARS)
    )
    c_lim = (
        comment_chars
        if comment_chars is not None
        else _int_env("LUFFY_ISSUE_COMMENT_CHARS", _DEFAULT_COMMENT_CHARS)
    )
    max_c = (
        max_comments
        if max_comments is not None
        else _int_env("LUFFY_ISSUE_CONTEXT_COMMENTS", _DEFAULT_COMMENTS)
    )
...
def assemble_from_pr_json(
    ...
    *,
    ...
    fixture_path = os.environ.get("LUFFY_ISSUE_CONTEXT_FIXTURE", "").strip()
    fixture: list[dict[str, Any]] | None = None
    if fixture_path:
