...
from PR size + model before the agent loop starts:
  ...
  - When LUFFY_MAX_COST_USD is unset: estimate-only (allow, write telemetry)
  - When budget is set and estimate exceeds it:
      force_cheap → switch to LUFFY_MODEL_CHEAP and re-estimate
...
Env:
  LUFFY_MAX_COST_USD          soft F29 budget; also hard preflight threshold when set
  LUFFY_PREFLIGHT_COST        on|off|auto  (default auto = hard when budget set)
  LUFFY_PREFLIGHT_ACTION      force_cheap|refuse|warn  (default force_cheap)
  LUFFY_MODEL_CHEAP           cheap model id (default openai/gpt-4.1-mini)
  LUFFY_PREFLIGHT_FORCE=1     always allow (paid run)
    ...
    _kv(
        {
            **est,
            ...
            "max_usd": parse_max_usd(
                ...
                else os.environ.get("LUFFY_MAX_COST_USD")
            ),
            "over_estimate": False,
            "forced_cheap": False,
            "refused": False,
            "cheap_model": os.environ.get("LUFFY_MODEL_CHEAP") or DEFAULT_CHEAP_MODEL,
            "mode": "estimate",
            "policy": "n/a",
        }
    )
...
def cmd_decide(args: argparse.Namespace) -> int:
    max_usd = parse_max_usd(
        args.max_usd if args.max_usd is not None else os.environ.get("LUFFY_MAX_COST_USD")
    )
    mode = parse_preflight_mode(
        args.mode if args.mode is not None else os.environ.get("LUFFY_PREFLIGHT_COST"),
        budget=max_usd,
    )
    action = parse_action(
        args.action if args.action is not None else os.environ.get("LUFFY_PREFLIGHT_ACTION")
    )
    model = (
        args.model
        or os.environ.get("LUFFY_MODEL")
    ...
    cheap = (
        args.cheap_model
        or os.environ.get("LUFFY_MODEL_CHEAP")
        or DEFAULT_CHEAP_MODEL
    )
    force = args.force or os.environ.get("LUFFY_PREFLIGHT_FORCE", "").strip() in (
        "1",
        "true",
        "yes",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    def add_common(sp: argparse.ArgumentParser) -> None:
        ...
        sp.add_argument(
            "--force",
            action="store_true",
            help="Always allow (LUFFY_PREFLIGHT_FORCE)",
        )
