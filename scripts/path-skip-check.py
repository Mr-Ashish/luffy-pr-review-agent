...
def parse_globs(raw: str | None) -> list[str]:
    """Parse LUFFY_SKIP_PATH_GLOBS. Empty/off → []. Preset `docs` → DOCS_PRESET."""
    if raw is None:
        return []
    s = str(raw).strip()
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--globs",
        default=None,
        help="Override LUFFY_SKIP_PATH_GLOBS (comma list or 'docs')",
    )
    args = p.parse_args(argv)

    if (os.environ.get("LUFFY_SKIP_PATHS_FORCE") or "").strip().lower() in (
        "1",
        "true",
        "yes",
    ):
    ...
    raw = args.globs if args.globs is not None else os.environ.get("LUFFY_SKIP_PATH_GLOBS")
