...
OpenUI phase 4b minimal: no hosted console required. Surfaces:
  ...
  - Optional LUFFY_CONSOLE_URL base (operator-hosted Run Console)
  - Always: download run-bundle.json from luffy-out artifact → ui/review-console
...
Env:
  LUFFY_OPS_FOOTER=1 (default) | 0/off to skip
  GITHUB_SERVER_URL, GITHUB_REPOSITORY, GITHUB_RUN_ID
  LUFFY_CONSOLE_URL — optional https://… base for interactive console
...
def enabled() -> bool:
    v = (os.environ.get("LUFFY_OPS_FOOTER") or "1").strip().lower()
...
def format_ops_line(
    *,
    ...
    console = (
        ...
        else (os.environ.get("LUFFY_CONSOLE_URL") or "").strip()
    )
    if console:
...
def format_step_summary(
    *,
    ...
    lines.append(
        "- **Interactive review:** download `luffy-out-*` artifact → "
