...
refresh content between <!-- luffy-description --> markers.
...
Env:
  LUFFY_PR_DESCRIPTION=1 (default) | 0/off — enable scaffold/apply helpers
  LUFFY_PR_DESCRIPTION_APPLY=0 (default) | 1 — allow gh pr edit on apply
  LUFFY_PR_DESCRIPTION_MODE=fill-empty|markers|force  (default fill-empty)
"""
...
MARKER_START = "<!-- luffy-description -->"
MARKER_END = "<!-- /luffy-description -->"
...
def enabled(raw: str | None = None) -> bool:
    try:
        ...
        v = raw if raw is not None else os.environ.get("LUFFY_PR_DESCRIPTION")
...
def apply_enabled(raw: str | None = None) -> bool:
    try:
        ...
        v = raw if raw is not None else os.environ.get("LUFFY_PR_DESCRIPTION_APPLY")
...
def mode_from_env() -> str:
    try:
        ...
        pass
    v = (os.environ.get("LUFFY_PR_DESCRIPTION_MODE") or "fill-empty").strip().lower()
    if v in ("fill-empty", "markers", "force"):
        return v
    return "fill-empty"
...
def is_placeholder_body(body: str | None) -> bool:
    ...
    stripped = re.sub(
        r"<!--\s*luffy-description\s*-->.*?<!--\s*/luffy-description\s*-->",
        "",
        b,
        flags=re.I | re.S,
    ).strip()
...
def main(argv: list[str] | None = None) -> int:
    ...
    pa.add_argument(
        "--force-apply",
        action="store_true",
        help="ignore LUFFY_PR_DESCRIPTION_APPLY=0",
    )
    ...
    if args.cmd == "apply":
        ...
        if args.dry_run or not (args.force_apply or apply_enabled()):
            ...
            result["reason"] = (
                "dry-run"
                if args.dry_run
                else "LUFFY_PR_DESCRIPTION_APPLY not enabled (use --force-apply)"
            )
