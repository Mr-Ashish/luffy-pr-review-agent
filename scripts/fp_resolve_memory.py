...
Deterministic control plane (no LLM):
  1. Mine author replies on Luffy inline threads (+ optional PR issue comments)
...
def enabled(raw: str | None = None) -> bool:
    if _toggle_enabled is not None:
        try:
            ...
            pass
    v = raw if raw is not None else os.environ.get("LUFFY_FP_RESOLVE")
...
def _is_bot_login(login: str) -> bool:
    ...
    if low.endswith("[bot]") or low.endswith("-bot"):
        return True
    if "luffy" in low or low in ("github-actions[bot]", "copilot-pull-request-reviewer[bot]"):
        return True
    return False
...
def patterns_from_review_comments(
    ...
    *,
    ...
    """Walk review comments; author replies to Luffy roots become FP patterns."""
    ...
    for c in comments:
        ...
        if parent_id is not None:
            try:
                ...
                parent = None
        # Prefer replies under Luffy roots; allow top-level author notes that cite a path
        if parent is not None:
            pbody = str(parent.get("body") or "")
            if not is_luffy_inline_body(pbody) and "luffy" not in pbody.lower():
...
def merge_into_memory(memory_md: str, patterns: list[FpPattern], *, max_bytes: int = 100_000) -> str:
    ...
    text = memory_md or "# Luffy review memory\n\n"
...
def fetch_review_comments(repo: str, pr: str | int) -> list[dict[str, Any]]:
    fixture = os.environ.get("LUFFY_FP_RESOLVE_FIXTURE", "").strip()
    if fixture:
...
def fetch_issue_comments(repo: str, pr: str | int) -> list[dict[str, Any]]:
    fixture = os.environ.get("LUFFY_FP_RESOLVE_ISSUE_FIXTURE", "").strip()
    if fixture:
        return json.loads(Path(fixture).read_text(encoding="utf-8"))
    data = _gh_json(
        [
...
def assemble(
    *,
    ...
    luffy_root = Path(os.environ.get("LUFFY_ROOT") or Path(__file__).resolve().parents[1])
    candidates.append(luffy_root / "agent" / "MEMORY.seed.md")
    ...
    try:
        ...
        src = "live" if not os.environ.get("LUFFY_FP_RESOLVE_FIXTURE") else "fixture"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "merge-memory":
        mem = args.memory.read_text(encoding="utf-8", errors="replace") if args.memory.is_file() else "# Luffy review memory\n"
