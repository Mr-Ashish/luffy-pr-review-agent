...
Env / var:
  LUFFY_MAX_TURNS  default 40; 0/off/false/no/none/disabled → unlimited (Hermes default)
"""
...
def effective_max_turns(cli_raw: str | None = None) -> int | None:
    """CLI/env precedence: explicit raw wins; else LUFFY_MAX_TURNS; else default 40."""
    if cli_raw is not None and str(cli_raw).strip() != "":
        return parse_max_turns(cli_raw)
    env = os.environ.get("LUFFY_MAX_TURNS")
