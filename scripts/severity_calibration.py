...
APPROVE, Luffy upgrades to REQUEST CHANGES (score cap aligned with the
...
Env:
  LUFFY_SEVERITY_CALIBRATION  1 (default) | 0/off
  LUFFY_SEVERITY_SCORE_CAP    default 69
...
def enabled(raw: str | None = None) -> bool:
    v = (
        raw
        if raw is not None
        else (os.environ.get("LUFFY_SEVERITY_CALIBRATION") or "1")
    )
    s = str(v).strip().lower()
...
def score_cap(raw: str | None = None) -> int:
    v = (
        raw
        if raw is not None
        else (os.environ.get("LUFFY_SEVERITY_SCORE_CAP") or str(DEFAULT_SCORE_CAP))
    )
    try:
        n = int(str(v).strip())
...
def banner_md(*, reason: str, match: str, snippet: str) -> str:
    ...
    return (
        ...
        f"Luffy upgrades to **REQUEST CHANGES** — missing tests for new "
...
def main(argv: list[str] | None = None) -> int:
    ...
    d.add_argument(
        "--enabled",
        default=None,
        help="Override LUFFY_SEVERITY_CALIBRATION (1/0)",
    )
