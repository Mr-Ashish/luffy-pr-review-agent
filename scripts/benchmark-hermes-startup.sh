#!/usr/bin/env bash
# F8: Benchmark Hermes startup paths used by Luffy CI.
#
...
PIN="$(LUFFY_HERMES_COMMIT="${LUFFY_HERMES_COMMIT:-}" "$PIN_HELPER" resolve | tr -d '\n')"
if [[ -z "$PIN" ]]; then
  PIN="$("$PIN_HELPER" default | tr -d '\n')"
fi
SHORT="${PIN:0:12}"
IMAGE_BASE="${LUFFY_RUNNER_IMAGE_BASE:-ghcr.io/mr-ashish/luffy-hermes-runner}"
IMAGE="${LUFFY_RUNNER_IMAGE:-${IMAGE_BASE}:${SHORT}}"
...
measure_cold() {
  ...
  local tmp t0
  tmp="$(mktemp -d "${TMPDIR:-/tmp}/luffy-hermes-cold.XXXXXX")"
  (
    export HOME="$tmp"
...
measure_tarball() {
  ...
  pack_tar="$(mktemp "${TMPDIR:-/tmp}/luffy-hermes-XXXXXX.tgz")"
  restore_home="$(mktemp -d "${TMPDIR:-/tmp}/luffy-hermes-restore.XXXXXX")"
  export PATH="${HOME}/.local/bin:${HOME}/.hermes/bin:${PATH}"
...
log "=== Luffy Hermes startup benchmark ==="
...
data = {
    "benchmark": "luffy-hermes-startup",
