...
Luffy control-plane (not Hermes fork). Turns run evidence + ROI backlog into
...
Usage:
  ...
  python3 scripts/agent_tools_pipeline.py toolsets   # print active LUFFY_TOOLSETS

Env:
  LUFFY_ROOT                 repo root (default: parent of scripts/)
  LUFFY_AGENT_TOOLS_AUTO_ADOPT=0|1  (default 0 — human gate for adopt)
"""
...
def cmd_adopt(args: argparse.Namespace) -> int:
    root = _root()
    auto = (os.environ.get("LUFFY_AGENT_TOOLS_AUTO_ADOPT") or "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def _write_active_toolsets(root: Path, catalog: dict[str, Any]) -> Path:
    """Comma-separated toolsets for LUFFY_TOOLSETS consumers."""
    sets: list[str] = []
    ...
    envp.write_text(f"LUFFY_TOOLSETS={','.join(sets)}\n", encoding="utf-8")
    return path
...
def main(argv: list[str] | None = None) -> int:
    ...
    sub.add_parser("toolsets", help="Print active LUFFY_TOOLSETS value").set_defaults(
        func=cmd_toolsets
    )
