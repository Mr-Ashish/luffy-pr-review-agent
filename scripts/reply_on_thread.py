#!/usr/bin/env python3
"""F60: reply on existing Luffy inline review threads.

When re-reviewing a PR, findings that land on the same path (+ optional line)
as a prior Luffy inline comment are posted as **thread replies**
...
Env:
  LUFFY_REPLY_ON_THREAD=1 (default on) | 0/off to skip
  LUFFY_REPLY_MATCH=path_line (default) | path  — match strictness
  LUFFY_REPLY_MAX=6
  LUFFY_REPLY_FIXTURE=path.json — write reply payload(s) offline (tests)
...
_LUFFY_INLINE_MARKERS = (
    "<!-- luffy-inline -->",
    "<!-- luffy-inline-review",
    "luffy inline findings",
    "🏴‍☠️ luffy inline",
)
...
def enabled(raw: str | None = None) -> bool:
    if _toggle_enabled is not None:
        try:
            ...
            pass
    v = raw if raw is not None else os.environ.get("LUFFY_REPLY_ON_THREAD")
    return _truthy(v, default=True)


def match_mode() -> str:
    m = (os.environ.get("LUFFY_REPLY_MATCH") or "path_line").strip().lower()
    return m if m in ("path_line", "path") else "path_line"


def max_replies() -> int:
    try:
        return max(1, min(20, int(os.environ.get("LUFFY_REPLY_MAX") or "6")))
    except ValueError:
        return 6
...
def is_luffy_inline_body(body: str) -> bool:
    if not body:
        return False
    low = body.lower()
    return any(m in low for m in _LUFFY_INLINE_MARKERS)
...
def is_luffy_thread(comment: dict[str, Any]) -> bool:
    """True if this PR review comment is a Luffy-owned root or reply."""
    ...
    if is_luffy_inline_body(body):
        return True
    user = comment.get("user") or {}
    login = str(user.get("login") or comment.get("user_login") or "").lower()
    if "luffy" in login or login.endswith("[bot]"):
        # only if body looks like ours
        if "luffy" in body.lower() or "severity" in body.lower():
            return True
    return is_luffy_inline_body(body)
...
def extract_thread_key(comment: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "is_luffy": is_luffy_thread(comment),
        "is_root": is_root_thread(comment),
    }


def index_luffy_roots(existing: list[dict[str, Any]]) -> list[dict[str, Any]]:
    roots: list[dict[str, Any]] = []
    for c in existing:
        key = extract_thread_key(c)
        if key["is_luffy"] and key["is_root"] and key["id"] is not None and key["path"]:
            roots.append(key)
    return roots
...
def format_reply_body(planned: dict[str, Any], *, run: str | None = None) -> str:
    ...
    header = "## 🏴‍☠️ Luffy follow-up"
    if sev:
        header += f" ({sev})"
    if kind == "suggestion":
        header += " · suggestion"
    run_bit = f" run={run}" if run else ""
    marker = f"\n\n<!-- luffy-inline-reply{run_bit} -->"
    # Avoid double markers bloating
    core = body
    if "<!-- luffy-inline -->" in core:
        core = core.replace("<!-- luffy-inline -->", "").rstrip()
    return f"{header}\n\n{core}{marker}"[:65000]
...
def plan_replies(
    ...
    *,
    ...
    roots = index_luffy_roots(existing)
...
def post_reply(
    ...
    fixture = (os.environ.get("LUFFY_REPLY_FIXTURE") or "").strip()
    if fixture:
        path = Path(fixture)
        existing: list[Any] = []
        if path.is_file():
